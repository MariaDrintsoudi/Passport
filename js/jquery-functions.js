$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
        currentLanguage === "greek"
            ? "Λυπούμαστε, αλλά δεν δικαιούστε να εκδώσετε Ελληνικό Διαβατήριο!"
            : "We are sorry, but you are not entitled to issue a Greek Passport!";
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
        ...
        // Save the answer to the first question
        if (currentQuestion === 0) {
            ...
            // Always proceed to the second question
            currentQuestion++;
            loadQuestion(currentQuestion, true);
            return;
        }

        if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
            currentQuestion = -1;
            currentLanguage === "greek" ? skipToEnd("Δεν μπορείτε να εκδώσετε Ελληνικό Διαβατήριο.") : skipToEnd("You cannot issue a Greek Passport.");
        } else {
